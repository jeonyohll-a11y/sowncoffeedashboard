...
function deeplUrl(authKey, env = {}) {
  if (env.DEEPL_API_URL) return env.DEEPL_API_URL;
...
async function translateTexts(texts, targetLang, authKey, env = {}) {
  const translated = [];
  for (const group of chunk(texts, 40)) {
    ...
    const response = await fetch(deeplUrl(authKey, env), {
      method: "POST",
      headers: {
...
async function handler(event) {
  ...
  const authKey = event.env?.DEEPL_API_KEY || "";
  if (!authKey) {
    return {
      statusCode: 500,
  ...
  try {
    ...
    const translations = textJobs.length
      ? await translateTexts(textJobs.map((job) => job.text), targetLang, authKey, event.env)
      : [];
